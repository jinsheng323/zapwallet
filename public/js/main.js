...
document.addEventListener(
  ...
  function() {
    if (
      document.querySelectorAll(".tabContainer").length &&
      document.querySelectorAll(".sectionsContainer").length
    ) {
      var activeTab = document
        .querySelector(".tabContainer")
        .querySelector(".tabs .tab:first-child");
      var activeSection = document
        .querySelector(".sectionsContainer")
  ...
  tabs: {
    ...
    setIndicatorWidth: function(parent) {
      ...
      indicator.style.right =
        containerRect.left +
        containerRect.width -
        (curTabRect.left + curTabRect.width) +
        "px";
    },
    setActiveTab: function() {
      ...
      var newSection = document.querySelector(
        ".sectionsContainer .sections .section[data-section=" +
          newTabSelector +
          "]"
      );
      var oldSection = document.querySelector(
      ...
      if (indicatorRect.left < newTabRect.left) {
        TweenMax.to(indicator, 0.2, {
          right: parentRect.left +
            parentRect.width -
            (newTabRect.left + newTabRect.width) +
            "px",
          ease: Power2.easeOut
        });
